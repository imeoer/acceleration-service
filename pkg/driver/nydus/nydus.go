...
func New(cfg map[string]string, platformMC platforms.MatchComparer) (*Driver, error) {
	...
	withTarToRafs, err := parseBool(cfg["with_tar_to_rafs"])
	if err != nil {
		return nil, errors.Wrap(err, "invalid with_tar_to_rafs option")
	}
